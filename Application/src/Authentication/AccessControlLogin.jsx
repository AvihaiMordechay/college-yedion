...
const AccessControlLogin = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (userUid === import.meta.env.VITE_FIREBASE_U1_UID) {
                const docRef = doc(db, "Admins", userUid);
                const admin = await getDoc(docRef);
                setUser(admin.data());
                navigate(`/ac-dashboard/${admin.data().personalId}`);
            }
        }
        catch (err) {
            console.log(err);
    ...
    const handleForgotPassword = async () => {
        ...
        if (email.length === 0) {
            ...
            return;
        } else if (email !== import.meta.env.VITE_FIREBASE_U1_EMAIL) {
